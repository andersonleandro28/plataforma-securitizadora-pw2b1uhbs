-- Eliminar sobrecarga da função revert_ccb_installment_liquidation (PGRST203)
-- PostgREST não resolve sobrecargas com parâmetros do mesmo nome em JSON (ex: p_recebivel_id text vs uuid).
-- Mantemos APENAS a assinatura com p_recebivel_id TEXT e p_installment_idx INT (convertendo internamente para UUID).
-- Removemos a assinatura com UUID.

DROP FUNCTION IF EXISTS public.revert_ccb_installment_liquidation(UUID, INT);
DROP FUNCTION IF EXISTS public.revert_ccb_installment_liquidation(TEXT, INT);
