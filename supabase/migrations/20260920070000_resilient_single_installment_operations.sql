-- 20260920070000_resilient_single_installment_operations.sql
-- Correção de resiliência para operações de crédito (especialmente parcela única):
-- 1. Backfill de installments_data para TODAS as operações de crédito com cronograma vazio/nulo e installments <= 1 (ou nulo)
-- 2. Recriação resiliente dos RPCs:
--    - revert_credit_operation_installment_liquidation
--    - liquidate_credit_operation_installment
--    - extend_credit_operation_installment
--    Garantindo que auto-inicializem o cronograma de 1 parcela quando vazio/nulo e installments <= 1,
--    e validando status de parcela de forma graciosa.

-- 1. Backfill de installments_data para operações com parcela única e cronograma vazio
DO $$
DECLARE
  r RECORD;
...
BEGIN
  FOR r IN 
    SELECT id, due_date, face_value, requested_value, status, liquidation_date, liquidation_value, installments_data
  ...
  LOOP
    v_is_paid := LOWER(COALESCE(r.status, '')) IN ('pago', 'liquidado');
    IF v_is_paid THEN
      v_status := 'pago';
    ELSE
      v_status := 'pendente';
    ...
    v_initial_inst := jsonb_build_array(
      jsonb_build_object(
        ...
        'value', COALESCE(r.face_value, r.requested_value, 0),
        'valor_original', COALESCE(r.face_value, r.requested_value, 0),
        'original_value', COALESCE(r.face_value, r.requested_value, 0),
        'status', v_status,
        'payment_date', CASE WHEN v_is_paid THEN COALESCE(r.liquidation_date::text, r.due_date::text, CURRENT_DATE::text) ELSE NULL END,
        'data_pagamento', CASE WHEN v_is_paid THEN COALESCE(r.liquidation_date::text, r.due_date::text, CURRENT_DATE::text) ELSE NULL END,
        'amount_paid', CASE WHEN v_is_paid THEN COALESCE(r.liquidation_value, r.face_value, r.requested_value, 0) ELSE NULL END
      )
    );
