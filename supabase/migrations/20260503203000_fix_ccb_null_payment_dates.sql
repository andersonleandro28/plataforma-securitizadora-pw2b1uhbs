...
BEGIN
  ...
  LOOP
    ...
    IF rec.installments IS NOT NULL THEN
      ...
      LOOP
        IF inst->>'status' IN ('paga', 'Pago', 'pago') AND (inst->>'data_pagamento' IS NULL AND inst->>'payment_date' IS NULL) THEN
          due_date := COALESCE(inst->>'data_vencimento', inst->>'due_date', inst->>'dueDate');
  ...
  LOOP
    ...
    IF rec.boletos IS NOT NULL THEN
      ...
      LOOP
        IF inst->>'status' IN ('Pago', 'pago', 'liquidado') AND (inst->>'data_pagamento' IS NULL AND inst->>'payment_date' IS NULL AND inst->>'data_liquidacao' IS NULL) THEN
          due_date := COALESCE(inst->>'data_vencimento', inst->>'due_date', inst->>'dueDate');
          IF due_date IS NOT NULL THEN
            inst := jsonb_set(inst, '{data_pagamento}', to_jsonb(due_date));
            changed := true;
