-- 20260921040000_fix_receivables_installments_liquidation.sql
--
-- 1. Reparar os dados das operações incorretamente marcadas como 'pago' / 'liquidado':
--    Operação só pode estar 'pago'/'liquidado' quando TODAS as parcelas do cronograma (installments_data)
--    têm status 'pago' ou 'liquidado'.
--    Se contiver parcelas em 'prorrogado', 'prorrogada', 'pendente' ou sem liquidação comprovada:
--      * Status volta para 'aprovado'.
--      * Limpar liquidation_date e liquidation_value.
--      * Estornar/remover apenas lançamentos decorrentes de quitação global indevida (op-liq-*),
--        preservando integralmente os lançamentos de parcelas individuais pagas (op-bol-*).
--
--    IDs afetados reportados especificamente:
--      - 1cdd539a-ee20-409e-847e-2fa45846700a (status 'pago' com 0/1 parcelas pagas, parcela 'pendente')
--      - fd52b616-7650-4a77-89d2-f6571e4cfa9a (status 'pago' com parcela 'prorrogado')
--      - 608f6c61-05cf-43e9-9822-9f6ab902a1dd (status 'pago' com parcela 'prorrogado')
--      - 8802936f-6b01-4ef3-a4f7-fd883e306d64 (status 'pago' com parcela 'prorrogado')
--      - e4b61cdb-8a47-48e7-b9c2-43c22aa17aad (status 'pago' com 4 parcelas 'prorrogado')
--      - e4d5271c-8019-4bb2-bec1-8948a17f1b80 (status 'pago' com 4 parcelas 'prorrogado')
--
--    Operações corretas preservadas (NÃO tocadas):
--      - 24ddd898-4afc-4aaa-92ac-52a026cbbae1 e d64ccab7-ce5c-4523-9b41-6633be86b2d0 (parcela prorrogada e depois PAGA)
--      - 841cc8dc-63dc-4ada-88dc-b5824202cb50 e 255621a2-fe9f-46a0-b328-54b3423e21a0 (parcela paga)
--      - 232b9f69-d485-4709-ad70-33aa20686ce4 (status 'reprovado')
--
-- 2. Corrigir o RPC liquidate_credit_operation_installment:
--    Garante que só considera parcela quitada se status in ('pago', 'liquidado').
--    Parcela 'prorrogada' ou 'pendente' NUNCA quita a operação. Idempotente.
--
-- 3. Atualizar extend_credit_operation_installment e revert_credit_operation_installment_liquidation
--    para manter a consistência rígida das regras.
...
BEGIN
  ...
  -- Montar novo cronograma limpando dados de quitação da parcela prorrogada
  ...
  -- Com a prorrogação desta parcela, a operação NÃO está totalmente liquidada.
  -- Se estava em 'pago' ou 'liquidado', reverter para 'aprovado'.
...
BEGIN
  ...
  LOOP
    ...
    -- REGRA ESTRITA: apenas parcelas com status 'pago' ou 'liquidado' (e que não estejam em prorrogado/pendente) contam como pagas.
    -- Parcela 'prorrogada' ou 'pendente' NUNCA quita a operação.
...
-- 4. Reparo dos dados das operações:
-- Regra estrita: operação só pode estar 'pago'/'liquidado' quando TODAS as parcelas do cronograma
-- têm status 'pago' ou 'liquidado'.
-- Se contiver parcelas em 'prorrogado', 'prorrogada', 'pendente' ou sem liquidação comprovada:
--   * Status da operação volta para 'aprovado'.
--   * Limpa liquidation_date e liquidation_value.
--   * Estorna lançamentos decorrentes de quitação global (op-liq-*), preservando lançamentos individuais (op-bol-*).
DO $$
DECLARE
  r RECORD;
  v_insts JSONB;
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- Se a parcela está com status 'prorrogado'/'prorrogada', ela não pode conter resquícios de payment_date
      ...
      -- Se não estiver explicitamente quitada ('pago' ou 'liquidado') com data de pagamento:
    ...
    -- Se a operação tem parcelas em aberto e está com status 'pago' ou 'liquidado':
    -- Voltar para 'aprovado' e limpar liquidation_date e liquidation_value.
    -- (Não toca se for 'reprovado' ou outro status não-pago).
    IF v_has_open_installments AND r.status IN ('pago', 'liquidado') THEN
      ...
      -- Estorna eventual quitação global op-liq-{id}, preservando op-bol-{id}-{n}
  ...
  RAISE NOTICE 'Reparo de recebíveis concluído: % operações corrigidas para status aprovado.', v_fixed_ops_count;
END $$;
