-- Fix the process_redemption_payment function to better link to the existing debenture subscription
...
DO $$
BEGIN
    -- 1. Atualiza as subscrições ligadas a investimentos que já estão totalmente resgatados ou cancelados
    UPDATE public.debenture_subscriptions ds
    SET status = 'Encerrado'
    FROM public.investments i
    WHERE ds.investment_id = i.id
      AND i.status IN ('resgatado', 'cancelled', 'Excluído')
      AND (ds.status IS NULL OR ds.status = 'Ativo');
      
    -- 2. Para investimentos parcialmente resgatados, atualiza a quantidade da subscrição de acordo
    UPDATE public.debenture_subscriptions ds
    SET quantity = GREATEST(0, i.quotas - COALESCE(i.redeemed_quotas, 0)),
        total_amount = GREATEST(0, (i.quotas - COALESCE(i.redeemed_quotas, 0)) * ds.unit_price)
    FROM public.investments i
    WHERE ds.investment_id = i.id
      AND i.status = 'approved'
      AND COALESCE(i.redeemed_quotas, 0) > 0
      AND ds.quantity > (i.quotas - COALESCE(i.redeemed_quotas, 0));
      
    -- 3. Por fim, se alguma quantidade ficou <= 0 em qualquer subscrição ativa, marca como encerrado
    UPDATE public.debenture_subscriptions
    SET status = 'Encerrado'
    WHERE quantity <= 0 AND (status IS NULL OR status = 'Ativo');
