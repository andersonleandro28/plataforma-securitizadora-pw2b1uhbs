ALTER TABLE public.investment_redemptions ADD COLUMN IF NOT EXISTS is_reinvestment BOOLEAN DEFAULT false;
ALTER TABLE public.investment_redemptions ADD COLUMN IF NOT EXISTS reinvestment_product_id UUID REFERENCES public.investment_products(id) ON DELETE SET NULL;
ALTER TABLE public.investment_redemptions ADD COLUMN IF NOT EXISTS reinvestment_quotas INTEGER DEFAULT 0;
...
BEGIN
    ...
    IF v_redemption.is_reinvestment AND v_redemption.reinvestment_product_id IS NOT NULL THEN
        ...
        -- Atualiza saldo investidor apenas com o troco
        UPDATE public.profiles
        SET wallet_balance = COALESCE(wallet_balance, 0) + v_troco
        WHERE id = v_profile.id;

        -- Cria novo investimento aprovado
        ...
        -- Adiciona subscrição na série do novo produto se existir
        ...
        VALUES ('out', v_redemption.net_value, 'Resgate (Base p/ Reinvestimento) - ' || COALESCE(v_profile.full_name, v_profile.pj_company_name, 'Investidor'), 'Resgates e Rendimentos', v_cat_resgate, CURRENT_DATE, p_admin_id);
        
        INSERT INTO public.treasury_transactions (type, amount, description, category, category_id, date, created_by)
        VALUES ('in', v_reinvest_amount, 'Integralização por Conversão de Crédito - ' || COALESCE(v_profile.full_name, v_profile.pj_company_name, 'Investidor'), 'Investimento', v_cat_invest, CURRENT_DATE, p_admin_id);
        
        IF v_troco > 0 THEN
            INSERT INTO public.treasury_transactions (type, amount, description, category, category_id, date, created_by)
            VALUES ('out', v_troco, 'Resgate de Sobra de Subscrição - ' || COALESCE(v_profile.full_name, v_profile.pj_company_name, 'Investidor'), 'Resgates e Rendimentos', v_cat_resgate, CURRENT_DATE, p_admin_id);
        END IF;

    ELSE
        -- Fluxo normal de resgate
        UPDATE public.profiles
