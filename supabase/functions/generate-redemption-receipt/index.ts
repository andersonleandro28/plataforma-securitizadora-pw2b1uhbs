...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    page.drawText(
      `Data da Liquidação: ${new Date(red.updated_at || red.created_at).toLocaleDateString('pt-BR')}`,
    ...
    page.drawText(
      ...
      { x: 50, y: 520, font, size: 12 },
    )
    if (red.penalty_applied > 0) {
      page.drawText(
        `Multa/Penalidade: R$ ${red.penalty_applied.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`,
        { x: 50, y: 500, font, size: 12, color: rgb(0.8, 0, 0) },
      )
    }
    if (red.discount_applied > 0) {
      page.drawText(
        `Deságio/IR: R$ ${red.discount_applied.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`,
        { x: 50, y: 480, font, size: 12, color: rgb(0.8, 0, 0) },
      )
    }
    page.drawText(
      `Valor Líquido Creditado: R$ ${red.net_value.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`,
      { x: 50, y: 440, font: fontBold, size: 14, color: rgb(0, 0.5, 0) },
    )
    ...
    page.drawText(
      ...
      { x: 50, y: 380, font, size: 9, color: rgb(0.3, 0.3, 0.3) },
    )
