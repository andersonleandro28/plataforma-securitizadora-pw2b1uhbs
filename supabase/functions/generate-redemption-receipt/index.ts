...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    page.drawText(`Data da Liquidação: ${new Date(red.updated_at || red.created_at).toLocaleDateString('pt-BR', { timeZone: 'America/Sao_Paulo' })}`, { x: 50, y: 740, font, size: 12 })
    ...
    if (red.is_reinvestment) {
      const reinvestAmount = red.reinvestment_quotas * (red.investments?.investment_products?.quota_value || red.investments?.investment_products?.min_investment || 1000)
      const troco = red.net_value - reinvestAmount
      
      page.drawText(`Valor Destinado a Reinvestimento: R$ ${reinvestAmount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`, { x: 50, y: curY, font: fontBold, size: 12, color: rgb(0, 0.3, 0.7) })
      curY -= 20
      page.drawText(`Troco Creditado em Conta: R$ ${troco.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`, { x: 50, y: curY, font: fontBold, size: 12, color: rgb(0, 0.5, 0) })
    } else {
