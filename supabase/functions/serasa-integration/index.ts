...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!document) {
      return new Response(JSON.stringify({ error: 'Documento (CPF/CNPJ) é obrigatório' }), {
        status: 400,
    ...
    if (docNumber.startsWith('000'))
      score = 150 // Critical score < 200
    ...
    return new Response(JSON.stringify({ data: mockResponse }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    })
  } catch (error) {
    return new Response(
