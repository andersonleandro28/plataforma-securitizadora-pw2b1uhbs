...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    currentY -= 10
    if (bankAccount) {
      currentY = drawTextWrap(`Dados Bancários para Crédito:`, margin, currentY, maxWidth, fontBold, size, page)
      ...
      if (bankAccount.pix_key) {
        currentY = drawTextWrap(`Chave PIX: ${bankAccount.pix_key}`, margin, currentY, maxWidth, font, size, page)
      }
    } else {
    ...
    if (op.status === 'aprovado') {
      await supabase.from('credit_operations').update({ status: 'aguardando_formalizacao' }).eq('id', op.id)
      await supabase.from('operation_status_history').insert({
        operation_id: op.id,
        old_status: op.status,
        new_status: 'aguardando_formalizacao',
        changed_by: user?.id,
        internal_observation: 'Status atualizado automaticamente após geração do aditivo.'
      })
    }
    ...
    if (bankAccount) {
      await supabase.from('audit_logs').insert({
        ...
        details: { message: `Aditivo ID ${op.id}: Dados bancários inseridos de usuário ${op.borrower_id}.` }
      })
    }
        ...
        body: JSON.stringify({
          ...
          html: `
            <div style="font-family: sans-serif; color: #333;">
              ...
              <p>Os dados bancários vinculados à operação foram incluídos no documento para conferência.</p>
              <p>Este documento também está disponível na sua conta da plataforma.</p>
              <br/>
