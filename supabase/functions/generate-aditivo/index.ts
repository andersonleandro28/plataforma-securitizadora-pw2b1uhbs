...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    currentY -= 20
    const cedenteNome = op.profiles?.pj_company_name || op.profiles?.full_name || op.cedente || 'N/A'
    ...
    // Mock Email Trigger
    console.log('--- ENVIO AUTOMÁTICO DE E-MAIL (FORMALIZAÇÃO) ---')
    console.log(`Para: ${op.profiles?.email}`)
    console.log(`Assunto: Aditivo de Cessão Gerado - Operação #${op.id.split('-')[0].toUpperCase()}`)
    console.log(`Mensagem: Olá! O aditivo da sua operação foi gerado. Acesse a plataforma para visualizá-lo e prosseguir com a assinatura digital.`)
    console.log('-------------------------------------------------')
    ...
    await supabase.from('audit_logs').insert({
        ...
        details: { email_to: op.profiles?.email, version: nextVersion }
    })
