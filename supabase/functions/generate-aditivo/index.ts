...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(
      `Assunto: Aditivo de Cessão Gerado - Operação #${op.id.split('-')[0].toUpperCase()}`,
    )
    console.log(
      `Mensagem: Olá! O aditivo da sua operação foi gerado. Acesse a plataforma para visualizá-lo e prosseguir com a assinatura digital.`,
    )
    console.log('-------------------------------------------------')
