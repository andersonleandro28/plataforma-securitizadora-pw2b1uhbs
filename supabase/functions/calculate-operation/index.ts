...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (operation_id && !simulate_data) {
      const { data, error } = await supabase.from('credit_operations').select('*').eq('id', operation_id).single()
    ...
    let params = paramsData?.find((p: any) => p.receivable_type === opData.receivable_type) || paramsData?.find((p: any) => p.receivable_type === 'global') || {}
    ...
    const structuring_val = params.structuring_fee_type === 'percentage' ? reqValue * (structuring_fee / 100) : structuring_fee
    const analysis_val = params.analysis_fee_type === 'percentage' ? reqValue * (analysis_fee / 100) : analysis_fee

    const iof_daily_val = reqValue * (iof_daily_rate / 100) * termDays
    const iof_fixed_val = reqValue * (iof_fixed_rate / 100)

    const total_discounts = discount_val + interest_val + ad_valorem_val + structuring_val + analysis_val + iof_daily_val + iof_fixed_val
    const net_value = reqValue - total_discounts
    const effective_cost = reqValue > 0 ? (total_discounts / reqValue) * 100 : 0

    const memory = {
      termDays, discount_val, interest_val, ad_valorem_val, structuring_val, analysis_val, iof_daily_val, iof_fixed_val, total_discounts, net_value, effective_cost, applied_params: params
    }

    if (simulate_data) {
      return new Response(JSON.stringify({ success: true, data: memory }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }

    if (operation_id) {
      const { data: calc, error: calcErr } = await supabase.from('operation_calculations').upsert({
        operation_id,
        term_days: termDays,
      ...
      }, { onConflict: 'operation_id' }).select().single()

      if (calcErr) throw calcErr

      return new Response(JSON.stringify({ success: true, data: calc }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    }
