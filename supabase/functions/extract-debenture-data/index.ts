...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (docType === 'investors') {
      ...
      profiles.push({
        full_name: 'Tomador Principal Alpha',
        email: 'tomador_alpha@corporativo.local',
        document_number: '11111111111',
        role: 'borrower',
      })
      profiles.push({
        full_name: 'Tomador Secundário Beta',
        email: 'tomador_beta@corporativo.local',
    ...
    for (let i = 1; i <= seriesCount; i++) {
      ...
      series.push({
        ...
        maturity_date: new Date(Date.now() + Math.random() * 100000000000)
          .toISOString()
          .split('T')[0],
      })
    }
