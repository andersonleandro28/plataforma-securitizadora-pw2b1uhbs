...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: op } = await supabase.from('credit_operations').select('id, status').eq('signature_envelope_id', envelope_id).maybeSingle()
    if (op) {
      ...
      const { data: profile } = await supabase.from('profiles').select('id, kyc_status').eq('kyc_signature_envelope_id', envelope_id).maybeSingle()
      if (profile) {
