...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: profileObj } = await supabaseAdmin.from('profiles').select('credit_limit').eq('id', user.id).single()
    const limit = Number(profileObj?.credit_limit || 0)

    const { data: ops } = await supabaseAdmin.from('credit_operations').select('requested_value').eq('borrower_id', user.id).not('status', 'in', '("liquidado","reprovado","cancelado","excluido")')
    const { data: ccbs } = await supabaseAdmin.from('ccb_solicitacoes').select('requested_value').eq('user_id', user.id).not('status', 'in', '("liquidado","reprovado","cancelado","excluido")')
    ...
    if (reqValue + usedOps + usedCcbs > limit) {
      throw new Error(`Limite de crédito excedido. Disponível: R$ ${Math.max(0, limit - usedOps - usedCcbs).toLocaleString('pt-BR')}`)
    }
