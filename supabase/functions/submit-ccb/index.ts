...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { borrowerData, operationData, guaranteesData, docsPaths, spouseData, guarantorData } =
      await req.json()
    ...
    page.drawText('SOLICITAÇÃO E SIMULAÇÃO DE EMISSÃO DE CCB', {
      x: margin + 150,
      y: currentY + 4,
      font: fontBold,
      size: 16,
    ...
    const attached =
      Object.keys(docsPaths || {})
        ...
        .map((k) => k.toUpperCase())
        .join(', ') || 'Nenhum'
    page.drawText(`Arquivos Base de KYC e Comprovação: ${attached.substring(0, 100)}`, {
      x: margin,
      y: currentY,
      font,
      size: 10,
    ...
    if (spouseData) {
      page.drawText(
        `Dados Cônjuge: ${sanitize(spouseData.name).substring(0, 40)} | CPF: ${sanitize(spouseData.document)}`,
        { x: margin, y: currentY, font, size: 10 },
      )
      page.drawText(`Nasc.: ${sanitize(spouseData.dob)} | Tel: ${sanitize(spouseData.phone)}`, {
        x: 400,
        y: currentY,
        font,
        size: 10,
      ...
      currentY -= 15
      if (docsPaths?.spouse_rg)
        page.drawText(`[X] Documentos do cônjuge anexados no sistema`, {
          x: margin,
          y: currentY,
          font,
          size: 9,
    ...
    currentY -= 15
    page.drawText(
      `Valor Solicitado: R$ ${Number(operationData?.requestedValue || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`,
    ...
    if (operationData?.simulation) {
      ...
      page.drawText(
        `Valor da Parcela Mensal: R$ ${Number(sim.installment_value || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`,
        { x: margin, y: currentY, font: fontBold, size: 10, color: rgb(0, 0.4, 0.7) },
      )
      page.drawText(
        `Total Projetado a Pagar: R$ ${Number(sim.total_to_pay || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`,
        { x: 300, y: currentY, font: fontBold, size: 10 },
      )
      currentY -= 15
      page.drawText(`Custo Efetivo Total (CET): ${Number(sim.cet || 0).toFixed(2)}% no período`, {
        x: margin,
        y: currentY,
        font,
        size: 10,
      ...
      page.drawText(
        `Taxa Mensal Aplicada: ${(Number(sim.rate_used || 0) * 100).toFixed(2)}% a.m.`,
        { x: 300, y: currentY, font, size: 10 },
      )
      currentY -= 15
      page.drawText(
        `Custo Fixo Emissão: R$ ${Number(sim.fixed_cost || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`,
        { x: margin, y: currentY, font, size: 10 },
      )
    } else {
    ...
    currentY -= 20
    page.drawText(`Garantia Exclusiva: ${sanitize(guaranteesData?.guaranteeType).toUpperCase()}`, {
      x: margin,
      y: currentY,
      font: fontBold,
      size: 10,
    ...
    page.drawText(
      `Recebível Vinculado: ${sanitize(guaranteesData?.receivableType).toUpperCase()}`,
      { x: 300, y: currentY, font, size: 10 },
    )
    currentY -= 15
    ...
    if (guarantorData) {
      page.drawText(
        `Avalista: ${sanitize(guarantorData.name).substring(0, 40)} | CPF: ${sanitize(guarantorData.document)} | Renda: R$ ${sanitize(guarantorData.income)}`,
        { x: margin, y: currentY, font, size: 10 },
      )
      currentY -= 15
      page.drawText(
        `Relação: ${sanitize(guarantorData.relationship)} | Tel: ${sanitize(guarantorData.phone)}`,
        { x: margin, y: currentY, font, size: 10 },
      )
      currentY -= 15
      if (docsPaths?.guarantor_rg)
        page.drawText(`[X] Documentos do avalista (RG, Renda, Endereço) anexados no sistema`, {
          x: margin,
          y: currentY,
          font,
          size: 9,
    ...
    if (docsPaths?.vehicleDoc) {
      page.drawText(`Documento do Veículo Anexado no Sistema`, {
        x: margin,
        y: currentY,
        font: fontBold,
        size: 10,
    ...
    currentY -= 20
    page.drawText(
      'Tomador submete a simulação e concorda com a análise de crédito via BDIGITAL e Plataforma Securitizadora.',
      { x: margin, y: currentY, font: fontBold, size: 10 },
    )
    ...
    page.drawText('Uso Exclusivo BDIGITAL / Securitizadora', {
    ...
    if (guarantorData) {
      ...
      if (docsPaths?.guarantor_rg)
        avalDocs.push({
          ccb_id: ccbId,
          nome_arquivo: 'RG/CPF Avalista',
          url: docsPaths.guarantor_rg,
        })
      if (docsPaths?.guarantor_income)
        avalDocs.push({
          ccb_id: ccbId,
          nome_arquivo: 'Comprovante de Renda',
          url: docsPaths.guarantor_income,
        })
      if (docsPaths?.guarantor_address)
        avalDocs.push({
          ccb_id: ccbId,
          nome_arquivo: 'Comprovante de Residência',
          url: docsPaths.guarantor_address,
        })

      if (avalDocs.length > 0) {
        await supabaseAdmin
          .from('ccb_avalistas_documentos')
          .insert(avalDocs)
          .catch((err) => console.error('Guarantor docs error', err))
      }
    }
