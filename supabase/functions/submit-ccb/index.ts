...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    currentY -= 20
    page.drawText(
      `Nome: ${sanitize(borrowerData?.name).substring(0, 50)} | CPF/CNPJ: ${sanitize(borrowerData?.document)}`,
    ...
    if (guarantorData) {
      drawH2('3. DADOS DO AVALISTA', currentY)
      currentY -= 20
      page.drawText(
        `Nome: ${sanitize(guarantorData.name).substring(0, 50)} | CPF: ${sanitize(guarantorData.document)} | Renda: R$ ${sanitize(guarantorData.income)}`,
        { x: margin, y: currentY, font, size: 10 },
      )
      currentY -= 15
      page.drawText(
        `Endereço: ${sanitize(guarantorData.street)}, ${sanitize(guarantorData.number)} - ${sanitize(guarantorData.city)}/${sanitize(guarantorData.state)}`,
        { x: margin, y: currentY, font, size: 10 },
      )
      currentY -= 20
    }
    ...
    currentY -= 15
    page.drawText(
      `Valor Solicitado: R$ ${Number(operationData?.requestedValue || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })} | Prazo: ${sanitize(operationData?.termMonths)} meses`,
      { x: margin, y: currentY, font, size: 10 },
    )
    currentY -= 15
    ...
    page.drawText(
      `Custo Fixo Emissão: R$ ${custoFixo.toLocaleString('pt-BR', { minimumFractionDigits: 2 })} | Parcela Mensal: R$ ${parcelaFinal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`,
      { x: margin, y: currentY, font, size: 10 },
    )
    currentY -= 15
    page.drawText(
      `Detalhamento IOF: Fixo (0.38%) R$ ${iofFixo.toLocaleString('pt-BR', { minimumFractionDigits: 2 })} + Diário R$ ${iofDiario.toLocaleString('pt-BR', { minimumFractionDigits: 2 })} = Total R$ ${totalIof.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`,
      { x: margin, y: currentY, font, size: 10 },
    )
    currentY -= 15
    ...
    if (bankData) {
      page.drawText(
        `Dados Recebimento: Banco ${sanitize(bankData.bank)} | Agência ${sanitize(bankData.branch)} | Conta ${sanitize(bankData.account)}`,
        { x: margin, y: currentY, font, size: 10 },
      )
      currentY -= 15
      page.drawText(
        `Titular: ${sanitize(bankData.owner_name)} (${sanitize(bankData.owner_document)}) | PIX: ${sanitize(bankData.pix_key)}`,
        { x: margin, y: currentY, font, size: 10 },
      )
      currentY -= 20
    }
    ...
    // DOCS SUMMARY
    drawH2('5. RESUMO DE DOCUMENTAÇÃO', currentY)
    currentY -= 20
    const keys = Object.keys(docsPaths || {})
      .map((k) => k.replace(/_/g, ' '))
      .join(', ')
    page.drawText(`Documentos enviados via sistema: ${keys.substring(0, 150)}...`, {
      x: margin,
      y: currentY,
      font,
      size: 10,
    ...
    if (spouseData) {
      await supabaseAdmin.from('usuarios_conjuges').insert({
        ...
        number: spouseData.number,
        neighborhood: spouseData.neighborhood,
    ...
    if (bankData) {
      await supabaseAdmin.from('dados_bancarios_ccb').insert({
        ...
        owner_name: bankData.owner_name,
        owner_document: bankData.owner_document,
