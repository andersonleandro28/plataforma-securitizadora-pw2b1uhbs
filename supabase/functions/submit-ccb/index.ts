...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    currentY -= 20
    page.drawText(`Tomador: ${sanitize(borrowerData.name)} | Documento: ${sanitize(borrowerData.document)}`, { x: margin, y: currentY, font, size: 10 })
    currentY -= 15
    ...
    currentY -= 20
    page.drawText(`Nome Completo: ${sanitize(borrowerData.name)}`, { x: margin, y: currentY, font, size: 10 })
    ...
    page.drawText(`Profissão: ${sanitize(borrowerData.occupation)}`, { x: 500, y: currentY, font, size: 10 })
    currentY -= 15
    ...
    page.drawText(`E-mail: ${sanitize(borrowerData.email)}`, { x: 500, y: currentY, font, size: 10 })
    currentY -= 15
    page.drawText(`Endereço: ${sanitize(borrowerData.street)}, ${sanitize(borrowerData.number)} - ${sanitize(borrowerData.neighborhood)} - ${sanitize(borrowerData.city)}/${sanitize(borrowerData.state)} - CEP: ${sanitize(borrowerData.zip)}`, { x: margin, y: currentY, font, size: 10 })
    currentY -= 30
    ...
    if (guaranteesData.sacados && guaranteesData.sacados.length > 0) {
      guaranteesData.sacados.slice(0, 4).forEach((s: any) => {
        page.drawText(`- Nome: ${sanitize(s.name)} | Documento: ${sanitize(s.document)} | Valor: R$ ${sanitize(s.value)}`, { x: margin + 10, y: currentY, font, size: 10 })
        currentY -= 15
      })
    ...
    console.log(`[EMAIL AUTOMÁTICO] Nova Solicitação de CCB BDIGITAL: ID ${ccbRecord.id} enviada por ${borrowerData.name}. PDF anexo e salvo em ${filePath}.`)
