...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: ccbs } = await supabaseAdmin
    ...
    if (reqValue + usedOps + usedCcbs > limit) {
      throw new Error(
        `Limite de crédito excedido. Disponível: R$ ${Math.max(0, limit - usedOps - usedCcbs).toLocaleString('pt-BR')}`,
      )
    }
    ...
    return new Response(
      JSON.stringify({ error: err.message || 'Erro interno no processamento da CCB' }),
