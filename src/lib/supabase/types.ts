...
//   status: text (nullable, default: 'Confirmado'::text)
...
//     WITH CHECK: true
// Table: credit_operations
//   Policy "credit_operations_delete_policy" (DELETE, PERMISSIVE) roles={authenticated}
//     USING: (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::app_role, 'staff'::app_role])))))
//   Policy "credit_operations_insert_policy" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: ((borrower_id = auth.uid()) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::app_role, 'staff'::app_role]))))))
//   Policy "credit_operations_select_policy" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: (((borrower_id = auth.uid()) AND (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'borrower'::app_role))))) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::app_role, 'staff'::app_role, 'accountant'::app_role]))))))
//   Policy "credit_operations_update_policy" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: ((borrower_id = auth.uid()) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::app_role, 'staff'::app_role]))))))
//     WITH CHECK: ((borrower_id = auth.uid()) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::app_role, 'staff'::app_role]))))))
// Table: dados_bancarios_ccb
...
//     WITH CHECK: true
// Table: investments
//   Policy "investments_delete_policy" (DELETE, PERMISSIVE) roles={authenticated}
//     USING: (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::app_role, 'staff'::app_role])))))
...
//     USING: (((user_id = auth.uid()) AND (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = 'investor'::app_role))))) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (profiles.role = ANY (ARRAY['admin'::app_role, 'staff'::app_role, 'accountant'::app_role]))))))
...
//   $function$
//   
// FUNCTION limpar_tabelas_problematicas()
//   CREATE OR REPLACE FUNCTION public.limpar_tabelas_problematicas()
//    RETURNS boolean
//    LANGUAGE plpgsql
//    SECURITY DEFINER
//   AS $function$
