...
//   rejection_reason: text (nullable)
// Table: mapeamento_movimentacoes
//   id: uuid (not null, default: gen_random_uuid())
//   movimentacao_caixa_id: uuid (not null)
//   origem_tabela: text (nullable)
//   origem_id: uuid (not null)
//   sincronizado: boolean (nullable, default: false)
//   data_sincronizacao: timestamp with time zone (nullable)
//   user_id: uuid (not null)
//   created_at: timestamp with time zone (nullable, default: now())
...
//   referencia_numero: text (nullable)
//   user_id: uuid (not null)
//   created_at: timestamp with time zone (nullable, default: now())
//   updated_at: timestamp with time zone (nullable, default: now())
...
//   saldo_anterior: numeric (nullable, default: 0)
//   saldo_novo: numeric (nullable, default: 0)
//   referencia_tipo: text (nullable)
...
// Table: auditoria_limpeza
//   PRIMARY KEY auditoria_limpeza_pkey: PRIMARY KEY (id)
//   FOREIGN KEY auditoria_limpeza_user_id_fkey: FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE SET NULL
// Table: border_items
...
// Table: mapeamento_movimentacoes
//   FOREIGN KEY mapeamento_movimentacoes_movimentacao_caixa_id_fkey: FOREIGN KEY (movimentacao_caixa_id) REFERENCES movimentacoes_caixa(id) ON DELETE CASCADE
//   CHECK mapeamento_movimentacoes_origem_tabela_check: CHECK ((origem_tabela = ANY (ARRAY['subscrições'::text, 'recebíveis'::text, 'ccb'::text, 'fornecedores'::text, 'despesas'::text, 'depósitos'::text, 'juros'::text])))
//   UNIQUE mapeamento_movimentacoes_origem_tabela_origem_id_key: UNIQUE (origem_tabela, origem_id)
//   PRIMARY KEY mapeamento_movimentacoes_pkey: PRIMARY KEY (id)
//   FOREIGN KEY mapeamento_movimentacoes_user_id_fkey: FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE
// Table: movimentacoes_caixa
//   CHECK movimentacoes_caixa_categoria_check: CHECK ((categoria = ANY (ARRAY['subscrição_debênture'::text, 'liquidação_recebível'::text, 'pagamento_ccb'::text, 'depósito'::text, 'juros_entrada'::text, 'fornecedor'::text, 'aquisição_ccb'::text, 'aquisição_recebível'::text, 'despesa'::text, 'juros_saída'::text])))
//   PRIMARY KEY movimentacoes_caixa_pkey: PRIMARY KEY (id)
//   CHECK movimentacoes_caixa_referencia_tipo_check: CHECK ((referencia_tipo = ANY (ARRAY['subscrição'::text, 'recebível'::text, 'ccb'::text, 'fornecedor'::text, 'despesa'::text, 'depósito'::text, 'outro'::text])))
//   CHECK movimentacoes_caixa_tipo_check: CHECK ((tipo = ANY (ARRAY['entrada'::text, 'saída'::text])))
//   FOREIGN KEY movimentacoes_caixa_user_id_fkey: FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE
// Table: operacoes_antecipacao
...
// Table: saldo_caixa
//   PRIMARY KEY saldo_caixa_pkey: PRIMARY KEY (id)
//   FOREIGN KEY saldo_caixa_user_id_fkey: FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE CASCADE
//   UNIQUE saldo_caixa_user_id_key: UNIQUE (user_id)
// Table: serasa_consultations
...
//     WITH CHECK: true
// Table: auditoria_limpeza
//   Policy "admin_all_auditoria" (ALL, PERMISSIVE) roles={authenticated}
//     USING: (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND ((profiles.role = 'admin'::app_role) OR (profiles.is_admin = true)))))
// Table: border_items
...
//     USING: (auth.uid() = user_id)
// Table: mapeamento_movimentacoes
//   Policy "mapeamento_movimentacoes_access_policy" (ALL, PERMISSIVE) roles={authenticated}
//     USING: ((user_id = auth.uid()) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND ((profiles.role = ANY (ARRAY['admin'::app_role, 'staff'::app_role, 'accountant'::app_role])) OR (profiles.is_admin = true))))))
//     WITH CHECK: ((user_id = auth.uid()) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND ((profiles.role = ANY (ARRAY['admin'::app_role, 'staff'::app_role, 'accountant'::app_role])) OR (profiles.is_admin = true))))))
// Table: movimentacoes_caixa
//   Policy "movimentacoes_caixa_access_policy" (ALL, PERMISSIVE) roles={authenticated}
//     USING: ((user_id = auth.uid()) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND ((profiles.role = ANY (ARRAY['admin'::app_role, 'staff'::app_role, 'accountant'::app_role])) OR (profiles.is_admin = true))))))
//     WITH CHECK: ((user_id = auth.uid()) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND ((profiles.role = ANY (ARRAY['admin'::app_role, 'staff'::app_role, 'accountant'::app_role])) OR (profiles.is_admin = true))))))
// Table: operacoes_antecipacao
...
//     USING: true
// Table: saldo_caixa
//   Policy "saldo_caixa_access_policy" (ALL, PERMISSIVE) roles={authenticated}
//     USING: ((user_id = auth.uid()) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND ((profiles.role = ANY (ARRAY['admin'::app_role, 'staff'::app_role, 'accountant'::app_role])) OR (profiles.is_admin = true))))))
//     WITH CHECK: ((user_id = auth.uid()) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND ((profiles.role = ANY (ARRAY['admin'::app_role, 'staff'::app_role, 'accountant'::app_role])) OR (profiles.is_admin = true))))))
// Table: serasa_consultations
...
//   $function$
//   
// FUNCTION check_duplicidade_mapeamento()
...
//   $function$
//   
// FUNCTION migrar_dados_historicos(numeric)
...
//   
//     FOR v_movs IN SELECT DISTINCT ON (origem_tabela, origem_id) * FROM temp_movs ORDER BY origem_tabela, origem_id, data_operacao ASC
//     LOOP
//       IF NOT EXISTS (SELECT 1 FROM public.mapeamento_movimentacoes WHERE origem_tabela = v_movs.origem_tabela AND origem_id = v_movs.origem_id) THEN
...
//   $function$
//   
// FUNCTION processa_movimentacao_caixa()
...
//   $function$
//   
// FUNCTION protect_movimentacao_core_fields()
...
//   $function$
//   
// FUNCTION recalcular_saldo_caixa(numeric)
...
//   $function$
//   
// FUNCTION sync_ccb_boletos_to_treasury()
...
//   $function$
//   
// FUNCTION sync_ccb_installments_to_treasury()
...
//   $function$
//   
// FUNCTION sync_investment_transfer_date()
...
//   $function$
//   
// FUNCTION sync_recebiveis_to_treasury()
...
// Table: investments
//   on_investment_transfer_date_changed: CREATE TRIGGER on_investment_transfer_date_changed AFTER UPDATE OF transfer_date ON public.investments FOR EACH ROW EXECUTE FUNCTION sync_investment_transfer_date()
// Table: mapeamento_movimentacoes
//   trg_check_duplicidade_mapeamento: CREATE TRIGGER trg_check_duplicidade_mapeamento BEFORE INSERT ON public.mapeamento_movimentacoes FOR EACH ROW EXECUTE FUNCTION check_duplicidade_mapeamento()
// Table: movimentacoes_caixa
//   trg_processa_movimentacao_caixa: CREATE TRIGGER trg_processa_movimentacao_caixa BEFORE INSERT ON public.movimentacoes_caixa FOR EACH ROW EXECUTE FUNCTION processa_movimentacao_caixa()
//   trg_protect_movimentacao_core_fields: CREATE TRIGGER trg_protect_movimentacao_core_fields BEFORE UPDATE ON public.movimentacoes_caixa FOR EACH ROW EXECUTE FUNCTION protect_movimentacao_core_fields()
// Table: operacoes_antecipacao
//   on_operacoes_antecipacao_change: CREATE TRIGGER on_operacoes_antecipacao_change AFTER INSERT OR UPDATE ON public.operacoes_antecipacao FOR EACH ROW EXECUTE FUNCTION sync_ccb_installments_to_treasury()
// Table: recebiveis_ccb
//   on_recebiveis_ccb_boletos_change: CREATE TRIGGER on_recebiveis_ccb_boletos_change AFTER UPDATE OF boletos ON public.recebiveis_ccb FOR EACH ROW EXECUTE FUNCTION sync_ccb_boletos_to_treasury()
//   on_recebiveis_ccb_change: CREATE TRIGGER on_recebiveis_ccb_change AFTER INSERT OR DELETE OR UPDATE ON public.recebiveis_ccb FOR EACH ROW EXECUTE FUNCTION sync_recebiveis_to_treasury()

// --- INDEXES ---
// Table: financial_parameters
//   CREATE UNIQUE INDEX financial_parameters_receivable_type_key ON public.financial_parameters USING btree (receivable_type)
// Table: mapeamento_movimentacoes
//   CREATE UNIQUE INDEX mapeamento_movimentacoes_origem_tabela_origem_id_key ON public.mapeamento_movimentacoes USING btree (origem_tabela, origem_id)
// Table: operacoes_antecipacao
...
// Table: saldo_caixa
//   CREATE UNIQUE INDEX saldo_caixa_user_id_key ON public.saldo_caixa USING btree (user_id)
// Table: transaction_categories
...
//   CREATE UNIQUE INDEX treasury_transactions_external_ref_key ON public.treasury_transactions USING btree (external_ref) WHERE (external_ref IS NOT NULL)
