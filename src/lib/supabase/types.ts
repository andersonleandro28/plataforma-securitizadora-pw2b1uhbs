...
//     USING: (auth.uid() = user_id)
// Table: mapeamento_movimentacoes
//   Policy "auth_mapeamento_all" (ALL, PERMISSIVE) roles={authenticated}
//     USING: ((auth.uid() = user_id) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND ((profiles.role = ANY (ARRAY['admin'::app_role, 'staff'::app_role, 'accountant'::app_role])) OR (profiles.is_admin = true))))))
//     WITH CHECK: ((auth.uid() = user_id) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND ((profiles.role = ANY (ARRAY['admin'::app_role, 'staff'::app_role, 'accountant'::app_role])) OR (profiles.is_admin = true))))))
// Table: movimentacoes_caixa
//   Policy "auth_movimentacoes_caixa_all" (ALL, PERMISSIVE) roles={authenticated}
//     USING: ((auth.uid() = user_id) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND ((profiles.role = ANY (ARRAY['admin'::app_role, 'staff'::app_role, 'accountant'::app_role])) OR (profiles.is_admin = true))))))
//     WITH CHECK: ((auth.uid() = user_id) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND ((profiles.role = ANY (ARRAY['admin'::app_role, 'staff'::app_role, 'accountant'::app_role])) OR (profiles.is_admin = true))))))
// Table: operacoes_antecipacao
...
//     USING: true
// Table: saldo_caixa
//   Policy "auth_saldo_caixa_all" (ALL, PERMISSIVE) roles={authenticated}
//     USING: ((auth.uid() = user_id) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND ((profiles.role = ANY (ARRAY['admin'::app_role, 'staff'::app_role, 'accountant'::app_role])) OR (profiles.is_admin = true))))))
//     WITH CHECK: ((auth.uid() = user_id) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND ((profiles.role = ANY (ARRAY['admin'::app_role, 'staff'::app_role, 'accountant'::app_role])) OR (profiles.is_admin = true))))))
// Table: serasa_consultations
