...
//     USING: ((user_id = auth.uid()) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (((profiles.role)::text = ANY (ARRAY['admin'::text, 'staff'::text, 'accountant'::text])) OR (profiles.is_admin = true))))))
//     WITH CHECK: ((user_id = auth.uid()) OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND (((profiles.role)::text = ANY (ARRAY['admin'::text, 'staff'::text, 'accountant'::text])) OR (profiles.is_admin = true))))))
// Table: movimentacoes_caixa
