...
//     USING: ((user_id = auth.uid()) OR is_admin() OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND ((profiles.role)::text = ANY (ARRAY['staff'::text, 'accountant'::text]))))))
//     WITH CHECK: ((user_id = auth.uid()) OR is_admin() OR (EXISTS ( SELECT 1    FROM profiles   WHERE ((profiles.id = auth.uid()) AND ((profiles.role)::text = ANY (ARRAY['staff'::text, 'accountant'::text]))))))
// Table: movimentacoes_caixa
